...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        // Use skip()
        //Read : https://howtodoinjava.com/java8/stream-skip-example/#:~:text=Stream%20skip(n)%20method%20is,skipped%20in%20the%20encounter%20order.
        ...
        // Use limit()
        // Read : https://stackoverflow.com/questions/31330330/fastest-way-to-get-the-first-n-elements-of-a-list-into-an-array
        ...
        // Use skip()
        // skip first 15 number to get last 5 number
...
//***************************************************************//
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().
                collect(averagingDouble(state -> state.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().
                mapToInt(state -> state.getCapital().getAverageYearlyPrecipitationInInches()).sum();
//         OR :
//        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().
//                collect(summingInt(state -> state.getCapital().getAverageYearlyPrecipitationInInches()));
...
// Read : https://javadeveloperzone.com/java-basic/java-8-stream-group-count/
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().
                collect(Collectors.groupingBy(state -> state.getCapital().getTimeZone() , counting()));
        ...
        String allStateNamesSemicolonDelimited = states.stream().
                map(stateInfo -> stateInfo.getStateName()).collect(Collectors.joining("; "));
        ...
        List<String> allDistinctStateBirds = states.stream().
                map(stateInfo -> stateInfo.getStateBird()).distinct().collect(toList());
...
//********************************************************************//
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().
                map(stateInfo -> stateInfo.getStateBird()).
 ...
 //Read : https://stackoverflow.com/questions/20995664/how-to-find-max-date-in-listobject
      //  Date maxDate = list.stream().map(u -> u.date).max(Date::compareTo).get();

        Integer maxStateElevation = states.stream().
                map(stateInfo -> stateInfo.getHighestElevationInFeet()).
        ...
        LocalDate earliestDateStateEnteredUnion = states.stream().
                map(stateInfo -> stateInfo.getDateAdmittedToUnion()).
...
// Read : https://stackoverflow.com/questions/48133547/using-java-8-stream-to-calculate-diference-between-two-values-and-return-the-min
        StateInfo stateWithLeastDistanceBetweenHighAndLowPoints = states.stream().
                min(Comparator.comparing(stateInfo -> Math.abs(stateInfo.getHighestElevationInFeet()-stateInfo.getLowestElevationInFeet()))).
                orElseThrow();
